...
parser = argparse.ArgumentParser(prog="Planar Circle Generator",
                                 description="Generate a 3DNA-formatted parameter file associated with a planar circle of DNA using trigonometric functions. Python3 requirements include: numpy and pandas modules. \nCode Generated by Robert Young, Rutgers University, 2021\n", 
...
def planar_circle(length, rise, bases_per_turn):
    ...
    * NOTE: if length / bases_per_turn (the linking number) is NOT an integer value, the ends of the circle will not smoothly connect
    ...
    for i in range(length):
        if i == 0:
            for PAR in ['rise','tilt','roll','twist']:
                DATAFRAME.at[i, PAR]=0.00000
        else:
...
def dataframe_pars_to_frames(DATAFRAME):
    ...
    DATAFRAME2.iloc[0]=[0.,0.,0.,1.,0.,0.,0.,1.,0.,0.,0.,1.]
    
    for i in range(1, len(DATAFRAME)):
        DATAFRAME2.iloc[i]=bp_geometry_conversion.steppar_to_frames(DATAFRAME.loc[[i]].to_numpy()[0], DATAFRAME2.loc[[i-1]].to_numpy()[0]).flatten()    
    del rf_titles
    return DATAFRAME2
...
def circular_dataframe_v2(DATAFRAME):
    DATAFRAME = pd.concat([DATAFRAME, DATAFRAME[0:1]])
...
def dataframe_frames_to_pars(DATAFRAME):
    ...
    for i in range(1, len(DATAFRAME)):
        DATAFRAME2.iloc[i]=bp_geometry_conversion.frames_to_steppar(DATAFRAME.loc[[i-1]].to_numpy()[0], DATAFRAME.loc[[i]].to_numpy()[0])
