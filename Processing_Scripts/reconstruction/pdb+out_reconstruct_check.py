...
if not os.path.exists(path+'/opt_out'):
    os.mkdir(path+'/opt_out')
 
os.chdir('opt_out')

subprocess.call(['x3dna_utils', 'cp_std', 'BDNA'])
for i in range(0, len(sorted(pars))):
    if not os.path.exists(path+'/opt_out/'+pars[i]+'.out'):
        shutil.copy(path+'/opt_par/'+pars[i]+'.par', path+'/opt_out')
